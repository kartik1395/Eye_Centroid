...
def find_eye(coords, gray, frame):
    '''Function to detect the pupil and centroid 
            using HoughCircles method'''

    # Extract the ROI (eye) detected by the haarcascade
    gray_roi = gray[coords[1]:coords[3], coords[0]:coords[2]]

    if len(gray_roi) == 0:
        return frame

    # Find the circles(pupil) within the ROI
    ...
    circles = np.uint16(np.around(circles))
    for i in circles[0, :]:
        # Draw the pupil and the centroid
        cv2.circle(frame, (i[0]+coords[0], i[1] +
                           coords[1]), i[2], (0, 255, 0), 2)
...
def haarcascade_method(gray, frame):
    '''Use the opencv available haarcascade for detection of eyes
            This becomes our ROI for extracting pupil and centroid'''
    eyes = eye_cascade.detectMultiScale(
        gray, 1.3, 5, minSize=(100, 100), maxSize=(300, 300))

    for (x, y, w, h) in eyes:

        cv2.putText(frame, "Eye", (x-20, y-30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--video", help='Enter full path of video file', required=True)
